...
- **Persistent shopper chrome.** Every public shopper screen carries the app top bar + site footer + bottom tab bar (HOME / SHOP / FOLLOWING / ACCOUNT); all three live in `apps/web/src/components/chrome/`, wrapped via `ShopperShell`. The **one shared top bar (`AppTopBar`) and the one shared footer (`SiteFooter`) are used on every page** — including the marketing landing (`/`), which renders both directly (no bottom tab bar). **Never hand-roll a page footer:** a screen inside `ShopperShell` already has one, and rendering another double-stacks it. Only the auth pages (§7 "The Tagged Feed" auth) stay chrome-free by design — they're a dead end, so the brand mark is the only way out.
