...
- **Persistent shopper chrome.** Every public shopper screen carries the app top bar + site footer + bottom tab bar (EXPLORE / FOLLOWING / SAVED — no Home, since the landing is a static page reached from the logo; no Account, since the top bar's avatar menu owns it on mobile too); all three live in `apps/web/src/components/chrome/`, wrapped via `ShopperShell`. The **one shared top bar (`AppTopBar`) and the one shared footer (`SiteFooter`) are used on every page** — including the marketing landing (`/`), which renders both directly (no bottom tab bar). The logo links to `/explore` for a signed-in visitor and to `/` (marketing) when signed out. **Never hand-roll a page footer:** a screen inside `ShopperShell` already has one, and rendering another double-stacks it. Only the auth pages (§7 "The Tagged Feed" auth) stay chrome-free by design — they're a dead end, so the brand mark is the only way out.
