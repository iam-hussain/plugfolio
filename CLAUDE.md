...
2. **Shopping never requires an account.** No login wall on any buy path. Accounts gate only: follow, save to the watchlist, comment (shopper), selling (creator), hiring (business).
...
| Auth | **Auth.js (NextAuth)**; **password login by email *or* member handle, email verified once at registration** (see [ADR-0004](./docs/adr/0004-creator-account-profiles-identity.md), [ADR-0012](./docs/adr/0012-password-login-with-registration-verification.md), [ADR-0024](./docs/adr/0024-username-at-verification-and-as-a-login.md)) | Creators, shoppers & businesses register with email + password; one verification email carries a link **and** a six-digit code, and verifying is where the account **picks its username** (reset by email link). Login then takes the email or that handle. A creator **account** connects Google+Meta and holds up to 5 **profiles**, each with a social-derived *profile* username (never a login); anonymous shoppers stay account-free. |
...
packages/
  ...
  tokens/         # "Plugfolio v2" design tokens (§7, ADR-0026) — CSS + TS, single source of color/space/type.
...
- **`app/` is thin — a route loads, it does not render.** A `page.tsx` holds `generateMetadata`, the param/searchParam parsing, the service calls and the auth check, then returns a single `<FeatureView …/>` from `features/`. **The page body is never JSX in `app/`**: it belongs in the feature that owns it, where it can be split, reused and put in Storybook. (`app/(public)/[handle]/page.tsx` was 386 lines of markup with nowhere to be tested; it is now 156 lines of loading plus `<CreatorPageView/>`.)
...
6. **Attribution is append-only.** Taps/clicks are immutable events (an event table), never mutated counters — the Traffic truth is a rebuildable projection (ADR-0021). Label `tracked` vs `estimated` at the read model, per the docs.
7. **No-login shopper identity = a signed device token**, not an account row. Anonymous by default; only follow/save/comment/creator/business create real user records.
...
## 7. Design system & theme — "Plugfolio v2"

The design source of truth is the interactive prototype **`Plugfolio v2.dc.html`** (Claude Design project), distilled for the codebase in [`docs/design/v2-visual-system.md`](./docs/design/v2-visual-system.md) and recorded in [ADR-0026](./docs/adr/0026-v2-visual-redesign.md) *(supersedes ADR-0016 "The Tagged Feed", amends ADR-0017)*. Centralize the theme; **components never hardcode hex, spacing, or fonts, and never use inline `style` attributes** — every visual value comes from a token utility class or a CVA variant.

**The world.** Plugfolio is the daylight side of the creator economy: a clean **white page**, oversized Sora headlines, Space-Mono micro-labels, and real photography inside violet-tinted cards and ink panels. Saturated colour arrives only as the page accent, the ink panels, and the photos themselves. The signature component is the **morphing pill nav** — a fixed, centred ink pill whose contents change by context. **Light is committed** (the `:root` default); dark is fully supported through the same tokens.

**Palette.** Raw values live in `@plugfolio/tokens` (`tokens.css`) as `--brand-*`; components consume the *semantic* tokens, never these.

| Role | Hex | Notes |
|---|---|---|
| Accent (default) | `#7C3AED` | violet — drives `--color-primary`; page-scoped, one of five |
| Accent options | indigo `#3D4EE8` · coral `#FF6B5C` · forest `#1C7A5C` · magenta `#C4247E` | pick one per page; everything downstream reads the token. Coral pairs with **ink** text (AA), the rest with white |
| Ink | `#12101C` | text; the pill-nav fill; ink panels; shadow colour |
| Electric Lime | `#C6FF3D` | **fill only, ink text on top** — flags a live offer, nothing else |
| Coral | `#FF6B5C` | the one danger / destructive colour |
| Forest | `#1C7A5C` | the one success colour |
| Canvas | `#FFFFFF` | the page ground (`--surface`) — white |
| Card | `#F8F7FB` | raised surfaces (`--surface-muted`); inset/"sunk" fields `#F1EFF7` (`--surface-active`) |

- **Page-is-white, card-is-the-tint rule (inverts the old "white is a lift").** The page ground `--surface` is white; raised objects step *up* into the violet-tinted `--surface-muted`, and inset fields sink to `--surface-active`. A card is a tint, not a shadow-only white plane.
...
- **One accent per page.** Saturated colour is the page accent (`--color-primary`, one of the five), the ink panels, and photography — never a random text colour, gradient, or full-bleed wash. The `--tile-*` hue tokens are **retired on shopper surfaces** (kept only for the admin app, deprecated).
- **Tokens are the only source of color/space/type.** Semantic tokens (`--color-primary`, `--color-accent`, `--surface`, `--surface-muted`, `--surface-active`, `--text`, `--text-muted`, `--text-faint`, `--border`, `--ring`) are defined once in `tokens.css` (light **and** dark) and exposed through the Tailwind preset in `@plugfolio/config`. **Reference tokens via classes, never raw hex.**
- **Type comes off the named scale — always.** `pico 10 · nano 11 · micro 12 · label 13 · copy 15 · body 17 · title 22 · name 24 · name-md 28 · name-lg 32`, plus the fluid display steps `display-sm · display · display-lg` for headlines. **From `copy` up the steps are fluid** (ADR-0022, amended): those numbers are the desktop end, and a 360px phone gets `copy 14 · body 15 · title 19 · name 20 · name-md 22 · name-lg 24`. That is the *only* place a size may respond to width — never a `sm:text-…` on a heading, which is the second scale wearing a breakpoint. **Never write `text-[13px]`, `text-[0.9375rem]` or a hand-rolled `clamp()`, and never Tailwind's own `text-xs/sm/base/lg/xl`** — those are a second scale, and a screen ends up on both. Need a size the scale hasn't got? Add a step to `packages/config/tailwind/preset.ts` with a one-line reason; don't inline it. (Only exception: shadcn primitives keep the `text-sm` they were generated with, so `shadcn add` diffs stay clean.)
- **One page container: `measure()` from `@plugfolio/ui`.** `measure()` = the 1200 measure + gutter, `measure({ width: "narrow" })` for long-form marketing, `measure({ width: "reading" })` for a column of prose. **Never hand-write `mx-auto w-full max-w-[…] px-…`** — that is how the shared top bar and the shared footer of the *same shell* ended up 20px out of alignment.
- **Radius:** climbs with the object — inputs 13–14px · cards 18–26px · the nav shell 26px · `pill 9999px` for anything interactive (`rounded-image|card|pill`; app UI also has `sm|md|lg`). **Anything interactive is a pill.** **Spacing:** 4-based (4 / 8 / 14 / 24 / 40 / 96).
- **Elevation:** soft and diffuse — `shadow-rest` (cards at rest), `shadow-tag` (tag pills on imagery), `shadow-lift` (hover), plus one deep soft step for the floating pill nav. No glass, no blur, no inner glow. **The old rotation-on-hover gesture is retired (ADR-0026):** v2 hover is **translate-up + accent border**.
- **Type, loaded via `next/font` in each app layout:**
  ...
  - **Inter** — UI & body (400–800), tabular nums for prices/counts (`font-sans`).
  - **Space Mono** — the label voice (promoted from code-only in v2): uppercase, letter-spaced micro-labels, eyebrows, price-adjacent metadata and status pills (`font-mono`), plus code/data.
- **Signature components (`@plugfolio/ui`):** the **morphing pill nav** (`PillNav`) — a fixed centred ink pill that swaps contents by context (browse tabs on list surfaces; back + Follow + share on a creator page; back + save + Buy on post/product; section tabs in the dashboard) — and `ProductTag` (the white pill pinned on a photo — name + tabular price + dot: violet affiliate / lime offer / violet-deep own; a real link, 44px tap target). `Button` is a pill that fills Ink and arrives at the page accent on hover.
...
- **Persistent shopper chrome.** Every public shopper screen carries the app top bar + site footer + the morphing pill nav (`PillNav`: Home / Shop / Follow / Saved / You), which context-swaps to a `PillNavOverride` on a creator/post/product surface; all live in `apps/web/src/components/chrome/`, wrapped via `ShopperShell`. The **one shared top bar (`AppTopBar`) and the one shared footer (`SiteFooter`) are used on every page** — except the marketing landing (`/`, `/for-creators`, `/for-business`), a full-bleed scroll story that carries its own scroll-revealed top bar and compact footer (ADR-0027). The logo links to `/explore` for a signed-in visitor and to `/` (marketing) when signed out. **Never hand-roll a page footer:** a screen inside `ShopperShell` already has one, and rendering another double-stacks it. Only the auth pages stay chrome-free by design — they're a dead end, so the brand mark is the only way out.
- **Storybook.** `pnpm --filter @plugfolio/web storybook` renders the design-system gallery. Stories live in `apps/web/stories/`; add a `.stories.tsx` when you add a shared component, and cover the *states that reshape it*, not just the default. `@storybook/addon-a11y` runs axe on the story you're looking at — **check its panel in both themes before you call a component done**, because that is the only place §7's AA rule gets tested at all (there is no CI gate for it). Note the `test` parameter is Storybook 9+; setting it here silently leaves the panel stuck on "Initializing…". A story may value-import `@plugfolio/core`: `node:crypto` is stubbed in `.storybook/stubs/`, so a barrel that reaches the signed device token (ADR-0002) no longer kills the whole preview.
- **Mobile-first & accessible:** design at 360px first (most visitors arrive in Instagram's in-app browser); hit WCAG AA in **both** themes (lime is fill-only for this reason); ≥44px hit targets; body never below 14px; no interaction depends on hover; respect `prefers-reduced-motion` (it also disables the system-screen PlugMark animation). Multi-card rows become a horizontal snap rail on phones.
...
- **Search the design system before you write markup.** `@plugfolio/ui` is large and its contents are not guessable — `CommentSection`, `Segmented`/`SegmentedOption`, `SortButton`, `DetailSectionHeading` and `measure()` all existed while pages re-typed them by hand, which is how the copies drifted off the design. Grep the barrel for the *thing* ("comment", "sort", "segment") before adding a `<div className="flex …">`.
- **Variants via `class-variance-authority` (CVA) — never hardcoded colors, never inline styles.** Any component with visual states (size, tone, active/inactive, kind) declares them with `cva`, keyed on token utility classes (`bg-primary`, `text-accent-foreground`, `border-border`, …). No raw hex, no `style={{ … }}` attribute, and no runtime-built Tailwind strings (`` `h-[${n}px]` ``) that the JIT can't see — use named CVA size/tone variants so every value stays on the token scale. Pick a variant with a prop; don't branch styling with ad-hoc `className` string concatenation. **A boolean prop that swaps classes is a variant**: `cond ? "bg-foreground" : "text-muted-foreground"` inside `cn()`, or a template literal with a ternary in it, is the same bug wearing a different shape.
- **Extend with `asChild`, never with an escape-hatch prop.** A component that lets a caller pass in a replacement for its own internals stops owning its layout, and every caller ends up pasting that layout back — `CreatorByline` had an `href` slot, so two pages each carried a copy of its avatar-and-name markup and its `name`/`handle` props were dead. Use Radix `Slot`/`Slottable` (see `BackLink`, `ProductSource`, `CreatorByline`).
- **Function components + hooks only.** One component per file; the file is named for the component.
- **A screen is not a component.** Past ~150 lines, split at the seam the design already has: one file per section (`account/`), the state and writes into a `use…` hook, the wording into a tested `lib/` helper. "One component, 350 lines" hides its own boundaries.
...
- [ ] Used shadcn/ui for anything it provides; searched `@plugfolio/ui` before writing markup; no custom component duplicating either.
- [ ] Theme via tokens, not hardcoded hex; mobile-first; AA contrast.
- [ ] Route holds loading only — the page body lives in its feature.
- [ ] Conditional styling is a CVA variant, not a ternary in `cn()` or a template literal.
- [ ] New/changed shared component has a story, and its a11y panel is clean in both themes.
- [ ] Types strict, functions small and named for intent.
- [ ] Journey/unit tests added; CI green.

**One grep catches most of the above** — it should print nothing:

```bash
grep -rnE 'text-\[(clamp|[0-9.]+(px|rem)\])|\btext-(xs|sm|base|lg|xl|[0-9]xl)\b|max-w-\[[0-9]{4}px\]|\? *"[^"]*(bg-|text-|border-)' \
  apps/*/src packages/ui/src --include='*.tsx' \
  | grep -v 'packages/ui/src/brand/' \
  | grep -v 'packages/ui/src/components/\(accordion\|alert\|avatar\|badge\|breadcrumb\|button\|calendar\|card\|chart\|checkbox\|combobox\|command\|context-menu\|dialog\|drawer\|dropdown-menu\|empty\|field\|form\|input\|item\|kbd\|label\|menubar\|native-select\|navigation-menu\|popover\|select\|sheet\|sidebar\|table\|tabs\|textarea\|toggle\|tooltip\)'
```

It catches off-scale type, a hand-rolled page measure, and a ternary picking token classes. The exclusion is the generated shadcn primitives — they keep their own scale so `shadcn add` stays diffable. A `max-w-[420px]` on a *component* (a dialog, a portrait video column) is fine and deliberately not matched; only a four-digit page measure is. Likewise a relative `text-[0.5em]` — sized off its parent by design — and `brand/`, whose lockup carries its own drawing ratio.
