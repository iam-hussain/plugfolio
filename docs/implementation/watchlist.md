...
> **Shown to shoppers as "Saved"** — the route is `/saved`, the top-bar nav and bottom tab both read "Saved". "Watchlist" is kept only as the internal/domain name (services, API `/api/watchlist`, `WatchlistRepository`, ADR-0025); nothing user-facing says it.
...
- No read endpoint: `/saved` server-renders the list, and the detail pages server-render the saved state.
...
- **`/saved`** (`app/(shopper)/saved/page.tsx` → `WatchlistPage`) — signed-in only, `redirect("/signin")` otherwise, inside `ShopperShell` like `/following`. One `DiscoveryGrid` of `DiscoveryCard`s — **the same card Explore uses** ([`explore.md`](./explore.md)), because a saved thing is the same object it was when you saved it: photo in its colour mat, **`@username`** in the byline (the creator who tagged it — half of why it was saved), title, then price / `Post` on the footer rule with `Open →` / `Buy →` / `Shop →` for the kind, and the offer flag on the photo. A `Bookmark` toggle sits in the card's corner, **outside** the card's link — the card is one link, and a button nested in a link is how someone unsaves by accident. Empty state points at `/explore`.
...
- **Chrome** — `Saved` in the signed-in top-bar nav, `Saved` (bookmark glyph) as a fifth bottom tab. `saved` is in `BASELINE_RESERVED_USERNAMES`, so no profile can shadow the route.
...
- Signed out on `/saved` → `/signin`. The shop path that led there is untouched.
...
- `apps/web/e2e/shopper.spec.ts` — `/saved` redirects anonymous visitors to sign-in and `POST /api/watchlist` answers 401.
