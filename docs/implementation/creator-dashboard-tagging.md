...
- `/dashboard` — profile switcher (`?profile=`), New-profile button, connect status, earnings for the active profile, nav to the four tabs.
- `/dashboard/posts` — post list (tag counts) + add-post form; each post opens the tagging editor.
- `/dashboard/posts/:postId` — the core tool: post preview, tagged products, tag form.
- `/dashboard/products` — every product of the profile (including ones whose post was deleted, via `listProducts`), inline affiliate-link fix + remove.
