...
- **Share row (DESIGN creator.html §.pshare / §.sh)** — "Share this page · Link · QR". Two *named* ways rather than one verb: "Share" alone made a visitor guess what would happen. Each opens `SharePanel` already on the mode they asked for, so nobody lands on a tab they have to switch away from.
  - It's a **modal, not a drawer**: a drawer stands beside the page because you're still working on it, and sharing is an errand you finish and leave. The shell is shadcn's `Dialog` (§8); the contents are the design system's `SharePanel` set.
  - Link mode: the URL with the handle picked out, a copy control that goes lime once it's done, the **unfurl preview built from the page's own avatar and counts** (so it can't drift from what actually renders when someone pastes it), and the ways out — Instagram first, deliberately, because the bio link is the one that matters and the one that needs the URL on the clipboard.
  - Code mode encodes for real via `@/lib/qr` (ported from the design's `qr.js`; `qr.test.ts` decodes every symbol back out, checks both format copies agree and the Reed-Solomon syndromes are zero). A code that doesn't scan would fail in exactly the moment it exists for — someone holding a phone up at a stall.
