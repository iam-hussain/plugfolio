...
- **Sponsored slot (ADR-0020)** — `AdPlacement` rows created in the admin app at `/sponsored`, read by `getLiveAdPlacement` and gated on the `ads` feature flag, which **defaults to false** (every other flag defaults on; an ad nobody remembered enabling is a worse failure than a missing one). The newest live placement wins, so replacing what's running means adding a new one rather than hunting for the old one first. No plan, no billing, no targeting.
